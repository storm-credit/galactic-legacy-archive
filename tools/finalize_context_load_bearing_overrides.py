#!/usr/bin/env python3
"""Final manual writer-route overrides for the CY751 epilogue load points.

The base load-bearing runner already enforces loss/payoff precedence. This thin
layer deepens E1097–E1099 where automatic extraction preserved outcomes but
...
_ORIGINAL_APPLY = runner.apply_load_bearing_override


def apply_final_epilogue_routes(a):
    a = _ORIGINAL_APPLY(a)
    ep = a["ep"]

    if ep == 1097:
        a["override_authority"] = (
            "MANUAL ENDING ROUTE — reconciled E1097 card + M-002 popular-history lock. "
            "Parus's permanent propulsion loss and current crew/service life must be visible through an ordinary trainee/current crew, not abstract nostalgia."
        )
        a["pov"] = (
        ...
        a["pov_auth"] = "M-002 LOCKED POPULAR-HISTORY POV + RECONCILED ENDING CARD"
        a["owner"] = (
        ...
        a["owner_auth"] = "M-002 + PARUS LOCKED END-STATE"
        a["decision"] = (
        ...
        a["decision_mode"] = "LOCKED EPILOGUE CURRENT-WORK + POPULAR-HISTORY DECISION"
        a["causal"] = (
            "PRESSURE[prove Parus survived socially without undoing its permanent strategic-propulsion loss and execute M-002 at ordinary scale] "
            "→ OBSTACLE[narrative nostalgia could secretly restore the old flagship role or replace plural memory with one corrected roster] "
            "→ PIVOT[current crew route the strategic-mobility task elsewhere; instructor lets an ordinary trainee record a sourced contested local version] "
            "→ DELTA[Parus remains a working route-school/rescue-service home and popular history remains plural] "
            "→ COST[speed, reach and old strategic possibility remain permanently lost] "
            "→ NEXT[the people who once shared the home must be encountered through their separate present work rather than a reunion summons]."
        )
        a["human"] = (
        ...
        a["human_auth"] = "M-002 LOCKED ORDINARY POV + CURRENT PARUS CREW CARRIER"
        a["relationship"] = (
        ...
        a["relationship_auth"] = "LOCKED PARUS END-STATE / NO-RECOLLECTION GUARD"
        a["payoff"] = (
        ...
        a["pov_auth"] = "WORKFLOW ROUTE FROM RECONCILED DISTRIBUTED-REUNION CARD — NOT NEW CANON"
        a["owner"] = (
        ...
        a["owner_auth"] = "RECONCILED E1098 INDEPENDENT-CURRENT-STATE PROOF"
        a["decision"] = (
        ...
        a["decision_mode"] = "WORKFLOW-NON-DISCRETE ENDING STATE FROM APPROVED CURRENT-WORK BLOCK"
        a["causal"] = (
            "PRESSURE[prove the old team can remain connected without institutional re-collection] "
            "→ OBSTACLE[a conventional finale would summon everyone around Rian and make independent institutions feel temporary] "
            "→ PIVOT[Nera, Serin, Mia and Haren are first shown doing their own current work and obligations; intersections remain bounded] "
            "→ DELTA[affection/history/trust persist while institutional ownership stays distributed] "
            "→ COST[distance and unresolved disagreement remain part of the stable ending] "
            "→ NEXT[Ern must prove an even stronger right: cooperation without final agreement or recruitment]."
        )
        a["human"] = (
        ...
        a["human_auth"] = "SOURCE-EXPLICIT INDEPENDENT-CURRENT-STATE PROOF"
        a["relationship"] = (
        ...
        a["relationship_auth"] = "RECONCILED E1098 RELATIONSHIP ARCHITECTURE"
        a["payoff"] = (
        ...
        a["pov"] = "Ern Varka through his own route/recovery/provenance institution and current constituency/mission constraints."
        a["pov_auth"] = "M-012 FINAL INDEPENDENCE PAYOFF / RECONCILED E1099"
        a["owner"] = (
        ...
        a["owner_auth"] = "M-012 LOCKED INDEPENDENT CURRENT MANDATE"
        a["decision"] = (
        ...
        a["decision_mode"] = "LOCKED INDEPENDENT-CURRENT DECISION"
        a["causal"] = (
            "PRESSURE[close Ern's arc without turning disagreement into recruitment or sequel bait] "
            "→ OBSTACLE[ending pressure could frame cooperation as final ideological surrender to Rian] "
            "→ PIVOT[Ern's own institution makes the current decision and keeps refusal available under documented conditions] "
            "→ DELTA[an opposition path becomes durable enough to outlive Rian's command] "
            "→ COST[the galaxy keeps institutions and arguments Rian cannot settle] "
            "→ NEXT[the final proof shrinks to one ordinary present person whose future importance cannot be queried]."
        )
        a["human"] = (
        ...
        a["human_auth"] = "M-012 INDEPENDENT-CURRENT ACTOR CARRIER"
        a["relationship"] = (
...
runner.apply_load_bearing_override = apply_final_epilogue_routes
