...
| ID | 결정 | 권장안 |
|---|---|---|
| **D1** | 목표 플랫폼/공모전 1개와 회차당 자수 기준(공백 포함 여부 명시) 확정. 현재 저장소에는 "5,500~6,500자 진단 범위"만 있고 플랫폼·산정 기준 결정 기록이 없음. project-charter의 "2026년 9월 공모전"까지 약 1개월. | 공모전 규정 확인 후 자수 기준을 decision-log에 D-entry로 기록 |
| **D2** | issue #26(인간/모바일 테스트)의 지위 확정. 현재 세 문서가 상호 모순: dry audit(집필 전 블로커) vs autonomous-override §4(출판 전 블로커) vs restoration §4(중립). | (b) 출판 전 블로커로 확정하고 첫 배치와 병행해 `gate8-human-mobile-test-packet-v1.md` 절차로 최소 5인 테스트 실행 |
| **D3** | 기존 `manuscript/ga1/` E1~20 비정본 초고 20편의 처리: (a) 참고 폐기 후 장면 카드에서 재집필 (b) 감사 통과 조건부 개정 기반으로 승격. 부수: NC 배너 삽입 또는 `manuscript/_archive_noncanon/` 이동(삭제 금지 원칙 유지). | (b) 개정 기반 — 20편이 이미 연속성·음성 감사 S0=0을 통과했고, 잔존 S1 5건은 압축 패스 대상으로 명시되어 있음 |
