...
Depends On: [[prewriting-gate]], [[prewriting-gate-dry-audit-2026-08-03]], [[prewriting-gate-evidence-matrix-v2]], [[design-only-scope-restoration-2026-08-03]], [[context-handoff-detail-complete-after-e1100-2026-08-05]], [[pre-writing-readiness-audit-v1]]
...
- 정본 게이트 문서는 [[prewriting-gate]](CANON).
...
유효 정본 우선순위는 [[design-only-scope-restoration-2026-08-03]] §3과 [[effective-canon-status-manifest-v1]] §6이 정의한다:
...
원고 문장은 존재만으로 정본을 만들 수 없다. 매니페스트가 레거시 헤더보다 우선한다(§1). 회차 집필 시 로드할 정본 파일 목록은 [[first-writing-batch-readiness-v1]] §2 참조.
...
| 검수·수정 절차 | READY | revision-harness 12패스 + [[manuscript-production-workflow-v1]](본 PR 신설) |
| 정본 계층·에라타 전파 | READY (S2 정정 필요) | 에라타 001/003/004 전역 전파 확인; 002는 미전파(§4) |

## 4. 미확정 항목 (게이트와 무관하게 수정해야 할 문서 결함)

전량 [[pre-writing-readiness-audit-v1]]의 S2 목록. 대표 항목:
...
9. [[continuity-issues]] 부재(revision-harness Pass 4가 참조).
...
| **D2** | issue #26(인간/모바일 테스트)의 지위 확정 | **해결 제안서 제출** — [[issue-26-status-resolution-proposal-v1]]. 제안: 출판 전 하드 블로커로 확정(과거 override 복권이 아닌 신규 작가 판정 방식), 첫 배치 병행 테스트 + 위험 통제 6항. 작가 승인 대기 |
...
- 상세 준비 상태는 [[first-writing-batch-readiness-v1]].
...
> **[2026-08-06 갱신] OPEN — 초고 생산 한정.**
>
> 작가가 D1~D4에 답하고 개방 선언을 기록했다. 공식 기록: [[pre-writing-gate-open-record-2026-08-06]]. 출판·공개는 인간 테스트(issue #26) 통과 전까지 계속 차단된다.
>
> (원판정 이력: 2026-08-06 오전 CLOSED 유지 — 설계 준비도는 집필 가능 수준이었으나 작가 결정 D1~D3과 명시 선언이 미기록 상태였음.)
