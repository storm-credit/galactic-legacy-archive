...
## D-20260813-04 — 등록부 노출 상한 폐기, 독자 기억 권위로 위임
Status: ACCEPTED
Date: 2026-08-13
Decision Owner: A00 (자체 레드팀 판정), 작가 확인 대기

### Context

작가가 물었다 — "설정집·설계도·세계관 문서 딥하게 정말 오케스트라가 판단해서 다 된 거야?" 확인 결과 세 가지가 사실이 아니었다 ([[registry-redteam-2026-08-13]]).

핵심은 D1이다. `CLAUDE.md` §12가 **새 이름을 만들기 전에 [[reader-facing-terminology-phonetics-and-register-bible-v1]]를 반드시 확인**하라고 요구하고 그 문서가 스스로를 **독자 기억 권위**로 규정하는데, 등록부 6종을 만들면서 그 문서를 발음 충돌표 한 줄 추가에만 썼다. 예산은 §2에 이미 있었다.

정본은 **회차당 총 고유명 최초 사용 0–4**. 내가 만든 것은 **장면당 합계 21**이었다. 국소 판단 여섯 번이 합쳐질 때를 보지 않았다.

### Decision

1. 등록부 6종의 자체 노출 상한을 **폐기**하고 권위 §2를 인용한다. 전면/배경 구분은 "예산 안에 들어올 자격"의 목록으로만 남긴다.
2. 어근이 진영은 구별하되 종류를 구별하지 못하므로(라틴 4도메인 61개, 그리스 4도메인 34개) **첫 등장에 종류어를 붙인다** — `전열함 유덱스`, `카빈 사기타`.
3. `tools/revision_checklist.py`가 도메인별 상한 대신 **회차당 최초 사용 0–4**를 센다.
4. 게이트를 `CONDITIONAL PASS` → **`HOLD`**로 되돌린다. BLOCKER 2건의 근거가 같은 날 같은 주체가 만든 문서다.
5. 등록부 6종의 `Owner Agents:` 역할명 헤더를 제거한다. **에이전트 ID가 없으므로 §15-1상 오케스트라가 아니다.**

### Consequences

- 등록부는 이제 예산을 갖지 않고 자격 목록만 갖는다. §3(단일 정본과 링크)에 맞다.
- 원고 검토가 정본이 세는 방식으로 센다.
- 게이트는 오늘 만든 문서가 **독립 검토**를 받기 전까지 열리지 않는다.

### Reversal Condition

권위 문서 §2가 개정되면 그 값을 따른다. 등록부가 다시 자기 숫자를 갖는 일은 없다.

## D-20260813-05 — 독립 오케스트라 검토 결과 반영
Status: ACCEPTED
Date: 2026-08-13
Decision Owner: A00 (독립 검토 결과 적용), 작가 확인 대기

### Context

작가 지시로 `$galactic-legacy-collection-orchestra`를 실제 실행했다. `codex exec --sandbox read-only`, 하위 에이전트 8(C1 Carson · C2 Nietzsche · C3 Erdos · C4 Planck · C5 Fermat · C6 Darwin · C7 Boyle · C8 Raman), 기준 커밋 `fa700c4`, 379,721 토큰, 파일 수정 없음. CLAUDE.md §15-1이 요구하는 실행 증거다.

**자체 레드팀(D-20260813-04)이 못 잡은 것을 독립 검토가 잡았다.** 자체 검토는 산수로 확인되는 것만 잡았고, 기존 정본과의 의미 충돌은 하나도 못 잡았다.

### Decision

전수 확인 후 다음을 적용한다 ([[registry-redteam-2026-08-13]] §5).

1. **ID 접두어 이동** — 무기 `A-` → `WA-`, 유물 `R-` → `RL-`. `A-001`은 [[antagonist-a001-continuity-director-v1]]의 정본 코드였고 `R-001`은 [[first-100-collectible-registry-v1]]에 이미 있었다.
2. **정본 명명 선체 1 → 3** — 손실 장부가 `L-S01 보루의 맹세`, `L-S02 온기`를 이미 잠가 뒀다. 함선 등록부 §3.0 신설, 총계 40 → 42.
3. **`F-06` → `PROPOSED — NONCANON`** — 원본이 그 블록만 비정본으로 표시한다.
4. **C9 검사 대상 210 → 239** — 접두어 로직이 `REC-F*` `AR-F*` `RT-*`를 건너뛰고 있었다.
5. **장소 등록부 자기모순 제거** — §1.1과 §7이 인구조사 CSV의 존재를 다르게 말했다.
6. **W10 주장 한정** — 07호 출력 45–55%·고부하 10–14분이므로 W10이 지탱하는 것은 구조·포획·개방 루프이며 지속 교전이 아니다.

### Consequences

- 유령 기준선을 네 개 잡아낸 문서가 **다섯 번째를 스스로 만들었다**(선체 1척 주장). 같은 주체는 그것을 볼 수 없었다.
- 검사 커버리지가 처음으로 전수(239/239)가 됐다.

### 미해결 (분리)

- 인구조사가 K-13·백색 도크·아르디스 규모, 노드 등급, `RT-04`/`RT-05` 직결과 충돌. `build_census.py` 배분 규칙 재검토 필요.
- 분야별 운용 필드 부족 (C3 탄약·인증, C5 운용, C6 제조·책임, C8 주민·거버넌스). 질감 필드 층.
- C6 `T-011`·`T-017`·`T-019` 회수 시점이 잠긴 GA10 회수와 충돌한다는 지적, 미검증.

### Reversal Condition

없음. ID 충돌과 상태 오기는 되돌릴 이유가 없다.

## D-20260813-06 — 작품 제목 《전몰 색인》
Status: ACCEPTED
Date: 2026-08-13
Decision Owner: 작가 (지시), A00 (4안 제시)

### Context

작가가 기존 제목이 SF답지 않다고 반려했다. 확인 결과 원인이 취향이 아니라 **범주 혼동**이었다 — `은하유산록`은 저장소에 82회 등장하는 **작중 유물의 이름**이며 작품 제목이 아니다. 물건 이름을 표지에 올리면서 `…록`이라는 사서·무협 어미가 함께 왔고 그것이 SF 신호를 지웠다.

### Options Considered

- **A 《전몰 색인》** — **채택.** 로그라인의 직역. `색인`은 이미 정본 개념(11건)이며 색인 삭제가 A-001의 핵심 플롯. `전몰`은 미사용어라 충돌 없음.
- B 《아카이브 07》 — SF 신호 최강이나 정보량 부족.
- C 《은하유산록: 전몰 색인》 — 물건 이름을 표지에 올리는 문제가 남음.
- D 《함대는 두 번 죽는다》 — 어감은 좋으나 수집 축이 제목에서 사라짐.

### Decision

작품명 **《전몰 색인》**. 작중 유물은 계속 《은하유산록》으로 부른다. 표지와 소품을 분리한다.
플랫폼 병기용 부제: 《전몰 색인 — 회귀한 패전 제독은 영웅을 모은다》.

### Consequences

- 저장소 이름 `galactic-legacy-archive`는 변경하지 않는다. 저장소는 작중 유물의 이름을 딴 것이고 그 사실은 여전히 맞다.
- 유물명 《은하유산록》 82회 노출은 그대로 유지된다. 개명이 아니다.

### Reversal Condition

플랫폼 노출 테스트에서 `색인`이 장르 신호로 읽히지 않으면 B 또는 D로 전환한다.
