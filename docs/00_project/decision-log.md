...
## D-20260819-01 — SLR 12편에 정본이 약속한 카드 비트를 승인한다

**채택**: 작가 승인 2026-08-19, [[slr-card-bit-proposals-2026-08-19]] 전 행. 하한 5,500자는 작가 기준으로 유지한다 — 개정안(나·다)은 철회됐다.

**실행**: E11(케이블 사출기 40초 + M-004 대비), E12(감시 표식 열람 비용 + W10 문법 인지 M-012), E13(네라 기여 조회 M-016), E14(저속 투사기 인증 대비), E15(방패판 점검 — 07호 미보행이므로 사용이 아니라 배치 계획), E16·17(한 호흡), E18(열한 계정 명단), E19(열절단창 90초 제약). E9·E10은 카드 여백 없음으로 미변경 유지.

**출처 규율**: 모든 수치는 운용 필드(케이블 40초, 냉각 90초)와 병합 정본(감시 표식, E7 조건, 11계정)에서 왔다. 신규 수치·능력·사망·관계 없음. W10 도구 4종의 질감 필드를 `SCENE_WRITTEN`으로 갱신한다.
...
## D-20260822-02 — 재집필 승인 배치를 E1–E20으로 연다

### Context

`D-20260822-01`로 E1–E100 초고 전체가 폐기됐고 `manuscript/`는 비어 있다. 승인 배치는 그 결정 시점에 열린 채로 남겨졌다.

같은 날 E011–E020 심화 Context Pack이 병합되면서(PR #221) E1–E20 전 구간이 회차별 깊은 실행 규격을 갖췄다.

| 구간 | Context 층 |
|---|---|
| E001–E005 | 회차별 수동 refresh 팩 |
| E006–E010 | [[ga1-e006-e010-context-pack-deep-v1]] |
| E011–E020 | [[ga1-e011-e020-context-pack-deep-v1]] |

직전까지 유효했던 승인 배치는 E001–E005였다([[first-writing-batch-readiness-v1]]).

### Options Considered

1. E001–E005 유지 — 폐기 전 범위를 그대로 쓴다.
2. E001–E010 — 심화 팩이 먼저 완성된 절반만 연다.
3. **E001–E020 — 심화 Context가 갖춰진 전 구간을 연다.**
4. 한 회차씩 승인 — 매 회차 작가 확인을 받는다.

### Decision

작가가 **3안**을 선택했다 (2026-08-22). 집필 실행은 Codex에 위임한다.

- 재집필 승인 배치: **E001–E020**.
- 회차 제목은 [[ga1-episodes-1-20-beat-map-v1]]의 승인 표제를 그대로 쓴다. 새 제목을 만들지 않는다.
- 파일은 `manuscript/ga1/NNN-제목-v1.md`, 헤더는 v2 스키마(`Status` / `Episode` / `Source Cards` / `Canon Check` / `Publication`).
- `Publication: NOT AUTHORIZED`를 유지한다. 이 결정은 **집필 승인이며 출판 승인이 아니다.**
- `AUTHOR-APPROVED`는 부여하지 않는다. 회차별 작가 승인은 별도 절차로 남는다.

### Reasons

- E1–E20 전 구간이 회차별 심화 Context를 갖췄다. 승인을 절반에서 끊으면 이미 준비된 규격이 놀게 된다.
- 20화는 [[ga1-episodes-1-20-beat-map-v1]]가 정의한 개막 계약의 단위다. E20에서 백색 도크 실배치가 시작되므로 그 지점이 자연스러운 배치 경계다.
- 원고가 전부 폐기된 상태라 개정 대상이 없다. 승인 범위를 잘게 끊어도 병합할 이전 버전이 없어 얻는 안전장치가 적다.
- 한 회차씩 승인하면 작가 확인이 20회 필요하다. 작가가 그 비용을 치르지 않기로 했다.

### Consequences

- E001–E020 원고 v1이 생산된다. `manuscript/` C3·C4·C5 검사가 다시 작동한다.
- [[episode-briefs]]의 `원고` 줄이 20회차분 복구된다.
- [[manuscript/README]] §4를 이 범위로 갱신한다.
- E021 이상은 여전히 승인되지 않는다.
- 공개·유료연재·출판은 별도 게이트다. issue #26 사람·모바일 검증은 출판 전 하드 블로커로 유지된다.

### Risks

- 20회차를 한 배치로 생산하면 초반 회차의 문체 판정을 받기 전에 후반 회차가 같은 습관을 반복할 수 있다.
- C4 하한(공백 포함 5,500자)이 집필 목표로 오용되어 패딩을 부를 수 있다 — CLAUDE.md §20이 금지하는 실패 유형이다.
- 심화 Context가 승인 장면 비트를 전부 들고 있으므로, 그것을 옮기는 데 그치고 장면이 산문으로 살아나지 않을 수 있다.
- §14에 기록된 출처 불일치 5건이 미해결 상태다. 해당 항목을 장면 사실로 실행하면 안 된다.

### Mitigation

- 배치를 5회차씩 끊어 생산하고, 각 배치마다 C3·C4·C5와 정본 대조를 돌린 뒤 다음 배치로 넘어간다.
- C4는 사후 검사로만 쓴다. 미달이면 문장을 늘리지 않고 승인 장면 비트가 빠졌는지 먼저 확인한다 (CLAUDE.md §20-5).
- [[ga1-e011-e020-context-pack-deep-v1]] §14의 미해결 항목(R-004 E11, F-011 E13, F-007 E13, M-012 시점)은 장면으로 실행하지 않는다.
- 원고 본문에 위키링크를 넣지 않는다. 링크는 헤더에만 둔다 (CLAUDE.md §14-5).

### Affected Documents

- [[manuscript/README]] §4
- [[first-writing-batch-readiness-v1]] — 승인 배치가 E001–E005에서 E001–E020으로 확대됨
- [[episode-briefs]] — 재생성
- [[decision-log]]

### Reversal Condition

작가가 범위를 좁히거나 특정 회차를 반려하면 이 항목을 갱신하고 [[manuscript/README]] §4를 함께 고친다. 생산된 원고는 삭제하지 않고 상태 필드로 처리한다.
